...
import {
  Engineer, CRQSchedule, TaskType, EngineerLevel, RequestorType, LEVEL_ORDER,
  TeamType, TEAM_TASKS, DOMAIN_SUBDOMAIN_MAP, ALL_DOMAINS
} from '@/types/crq';
...
const ManualScheduleDialog = ({ engineers, schedules, onSchedule }: ManualScheduleDialogProps) => {
  ...
  // Derived task options based on selected team
  const availableTasks: TaskType[] = useMemo(() => {
    if (!team) return [];
    return TEAM_TASKS[team as TeamType];
  }, [team]);

  // Derived subdomains based on selected domain
  const subdomains = useMemo(() => {
    if (!domain) return [];
    return DOMAIN_SUBDOMAIN_MAP[domain] || [];
  }, [domain]);

  // Filter engineers based on all cascading filters
  const filteredEngineers = useMemo(() => {
    let engs = [...engineers].sort((a, b) => LEVEL_ORDER[a.level] - LEVEL_ORDER[b.level]);
    if (team) engs = engs.filter(e => e.team === team);
    if (domain) engs = engs.filter(e => e.domain === domain);
    if (subdomain) engs = engs.filter(e => e.subdomain === subdomain);
    if (level) engs = engs.filter(e => e.level === level);
    if (taskType) engs = engs.filter(e => e.skills.includes(taskType as TaskType));
    return engs;
  }, [engineers, team, domain, subdomain, level, taskType]);
  ...
  const checkConflict = () => {
    ...
    const overlapping = schedules.find(s => s.engineerId === engineerId && !(end <= s.startHour || start >= s.endHour));
    if (overlapping) {
      ...
      for (let h = eng.shiftStart; h + duration <= eng.shiftEnd; h++) {
        ...
        if (free) { setSuggestion(`Nearest free slot: ${h}:00–${h + duration}:00`); return; }
      }
      setSuggestion('No free slots available in this shift');
      return;
    }
    setConflict(null);
    setSuggestion(null);
  ...
  const handleSubmit = () => {
    if (!crqNumber || !engineerId || !startHour || !endHour || !taskType || !team) {
    ...
    const newSchedule: CRQSchedule = {
      ...
      subdomain: subdomain || eng.subdomain,
  ...
  const resetForm = () => {
    setCrqNumber(''); setTeam(''); setTaskType(''); setDomain(''); setSubdomain('');
    setLevel(''); setEngineerId(''); setStartHour(''); setEndHour('');
    setRequestor(''); setConflict(null); setSuggestion(null);
  };

  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
      <DialogTrigger asChild>
        <Button size="sm" className="gap-1.5"><Plus className="h-3.5 w-3.5" /> Manual Slot Allocation</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-lg bg-card border-border max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {/* Step 1: Team + Task (dependent) */}
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label className="text-xs">Team *</Label>
              <Select value={team} onValueChange={v => { setTeam(v); setTaskType(''); setEngineerId(''); setConflict(null); }}>
                <SelectTrigger className="bg-secondary"><SelectValue placeholder="Select Team" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={taskType} onValueChange={v => { setTaskType(v); setEngineerId(''); setConflict(null); }} disabled={!team}>
                <SelectTrigger className="bg-secondary"><SelectValue placeholder={team ? 'Select Task' : 'Select Team first'} /></SelectTrigger>
                <SelectContent>
          ...
          {/* Step 2: Domain + Subdomain (dependent) */}
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              <Label className="text-xs">Domain</Label>
              <Select value={domain} onValueChange={v => { setDomain(v); setSubdomain(''); setEngineerId(''); setConflict(null); }}>
                <SelectTrigger className="bg-secondary"><SelectValue placeholder="All Domains" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={subdomain} onValueChange={v => { setSubdomain(v); setEngineerId(''); setConflict(null); }} disabled={!domain}>
                <SelectTrigger className="bg-secondary"><SelectValue placeholder={domain ? 'Select Subdomain' : 'Select Domain first'} /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="">All</SelectItem>
          ...
          <div className="space-y-2">
            ...
            <Select value={level} onValueChange={v => { setLevel(v); setEngineerId(''); setConflict(null); }}>
              <SelectTrigger className="bg-secondary"><SelectValue placeholder="All Levels" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="">All</SelectItem>
          ...
          {/* Engineer dropdown - filtered by above */}
          <div className="space-y-2">
            <Label className="text-xs">Engineer <span className="text-muted-foreground">({filteredEngineers.length} matching)</span></Label>
            <Select value={engineerId} onValueChange={v => { setEngineerId(v); setConflict(null); }}>
              ...
              <SelectContent>
                {filteredEngineers.map(e => {
                  const engSch = schedules.filter(s => s.engineerId === e.id);
                  const busyHrs = engSch.reduce((sum, s) => sum + (s.endHour - s.startHour), 0);
                  const freeHrs = (e.shiftEnd - e.shiftStart) - busyHrs;
                  return (
                    <SelectItem key={e.id} value={e.id}>
                      <div className="flex items-center gap-2">
                        <span>{e.name}</span>
                        <span className="text-muted-foreground text-[10px]">({e.level} · {e.team})</span>
                        <span className="text-[10px] text-status-free">{freeHrs}h free</span>
                        <span className="text-muted-foreground text-[10px]">{e.shiftType}</span>
                      </div>
                    </SelectItem>
                  );
                })}
          ...
          {selectedEngineer && (
            <div className="rounded-md bg-secondary/50 px-3 py-2 text-xs text-muted-foreground">
              <span className="font-medium text-foreground">{selectedEngineer.name}</span> · {selectedEngineer.domain} / {selectedEngineer.subdomain} · <span className="text-foreground">{selectedEngineer.shiftType}</span> ({selectedEngineer.shiftStart}:00–{selectedEngineer.shiftEnd}:00) · Skills: {selectedEngineer.skills.join(', ')}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Select value={startHour} onValueChange={v => { setStartHour(v); setConflict(null); }}>
                ...
                <SelectContent>
                  {timeOptions.slice(0, -1).map(h => <SelectItem key={h} value={String(h)}>{h}:00</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              ...
              <Select value={endHour} onValueChange={v => { setEndHour(v); setConflict(null); }}>
                ...
                <SelectContent>
                  {timeOptions.filter(h => h > parseInt(startHour || '0')).map(h => <SelectItem key={h} value={String(h)}>{h}:00</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
